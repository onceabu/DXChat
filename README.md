...
# 总结
APP已在应用平台上线，篇幅原因还有一些细节没有进行说明和展示，详细内容可直接下载应用使用并了解。<br>
同时应用目前还只是为了实现而实现，缺少一些新意，也存在很多不足，由于自己现在还在银行上班，平时时间匮乏，只能在下班之后或者偶尔休息的时候才能敲一敲键盘，如果不是自己对Android的喜好也不会坚持到现在，下一步会将APP架构进一步完善，同时把相关功能组件和UI组件进行封装复用，也希望自己能改变自己，尽快把精力和激情放到自己想做的事情上。
